...
function Input({ className, type, ...props }: React.InputHTMLAttributes<HTMLInputElement>) {
  return (
    <input
      ...
      data-slot="input"
      className={cn(
        "w-full min-h-10 rounded-default border border-outline bg-surface-container-low px-3 py-2 text-body-md text-on-surface shadow-xs transition-colors outline-none placeholder:text-on-surface-variant hover:border-outline-variant focus:border-primary focus:shadow-focus-ring disabled:cursor-not-allowed disabled:opacity-50",
